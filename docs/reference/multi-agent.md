...
| `Pi` | per-worker terminal harness 후보. session tree/context engineering은 Pi가 소유하고, awf는 gate/state/provenance만 소유 | 감지 + print-mode `AgentResult` bridge 구현, dispatch backend 연결은 계획 |
