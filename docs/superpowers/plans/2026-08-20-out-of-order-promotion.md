...
- `cli/src/awf/worktrees/models.py`: promotion and resolution enums plus durable lease fields and JSON shape.
- `cli/src/awf/worktrees/registry.py`: backward-compatible SQLite migration, inserts, CAS transitions, and row mapping.
- `cli/src/awf/worktrees/git.py`: structured three-way patch conflicts, unmerged-path inspection, and AWF-owned staging.
- `cli/src/awf/cli.py`: `--out-of-order` parser surface.
- `cli/src/awf/commands/wt.py`: CLI-to-service forwarding.
- `cli/src/awf/worktrees/service.py`: validation, lease identity, automatic synthetic promotion, conflict preservation, resolution preview/apply, provenance, and publication.
- `cli/tests/test_worktree_registry.py`: new and legacy registry contracts.
...
- `cli/tests/test_worktree_service.py`: automatic and manual out-of-order behavior plus exact-mode regressions.
...
Add a CAS transition test that changes `resolution_state` and `conflicted_paths` and survives a reload.
...
Use exact/none/`None`/empty-tuple defaults in `Lease.new()`. Convert enum values and both tuples to JSON lists in `to_dict()`.

- [ ] **Step 4: Add idempotent SQLite migration and mappings**

Extend `_SCHEMA` with checked mode/state columns, nullable SHA columns, and JSON text path columns. In `ensure()`, after `executescript`, inspect `PRAGMA table_info(worktree_leases)` and conditionally execute these additions for existing databases:
...
Use `json.dumps(list(paths), separators=(",", ":"))` on write and validate decoded values are sorted unique strings on read. Reject malformed registry data with `ValueError`; do not silently discard it.

Extend `transition()` with optional `resolution_state` and `conflicted_paths` arguments. Update only fields explicitly supplied, preserving CAS semantics.
...
Return the blocked lease and a concise message listing the paths. Do not reset, commit, push, or create a PR.
