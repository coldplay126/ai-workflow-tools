...
- `awf agents followup-omp`는 provenance inspect와 실행 경계를 분리한다. JSON inspect는 read-only이며, 실행 시 persisted OMP host session을 resume해 exact task ID에 `hub send`로 먼저 steer/revive한다. registry agent가 unavailable일 때만 exact `history://` handle에서 새 successor task 한 개를 만들고 parent run/task lineage를 기록한다. successor는 original agent와 동일시하지 않으며 worker/follow-up은 `.workflow/state.json`, scope hash, gate, approve/done을 수정하지 않는다
