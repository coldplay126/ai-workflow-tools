...
def _resolve_analysis_resume_locked(context, current_file_entries, deps) -> dict:
    ...
    if domain_bundle and saved_hash and saved_hash != config_hash:
    ...
    if project_bundle and saved_hash and saved_hash != config_hash:
    ...
    # K0: Check file hashes change before deciding to skip
    ...
    if output_status == "completed" and deps["output_files_present"](context) and saved_hashes:
        if files_changed:
    ...
    if stage3_status == "failed":
        ...
        if stage3_retry_count >= 2:
            stage3_retry_blocked = True
            messages.append(f"resume: stage3 retryCount={stage3_retry_count}; automatic retry is blocked.")
        else:
            # Clean up failed Stage 3 artifact
            stage3_artifact = state.get("artifacts", {}).get("stage3_final")
            if stage3_artifact:
                stage3_path = context.ai_context_dir / stage3_artifact
...
def finalize_analysis_run(context, provider_name: str, returncode: int, error_message: str = "") -> dict:
    from awf.core.analysis_state import analysis_state_path
    deps = _deps()
    lock = _analysis_lock(analysis_state_path(context))
    with lock:
        return _finalize_analysis_run_locked(context, provider_name, returncode, error_message, deps)


def _finalize_analysis_run_locked(context, provider_name: str, returncode: int, error_message: str, deps) -> dict:
    state = deps["load_state"](context)
    if returncode == 0 and deps["output_files_present"](context):
        ...
        resolved_error = error_message or existing_stage2_error or "Provider run completed without required outputs."
        ...
        state["layers"]["output"]["errorMessage"] = error_message or existing_output_error or "Required .ai-context outputs not found."
