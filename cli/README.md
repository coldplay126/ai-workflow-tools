...
- `awf wf next [--phase <name>] [--mode solo|quick|precise|cross|critical] [--auto-apply] [--non-interactive] [--no-ready-gate]`: 다음 provider-backed phase 해석, delegated prompt 생성, `.workflow/tmp/`에 prompt/result 저장, fallback chain 시도, phase를 `in_progress`로 표시한다. Provider-backed 실행은 기본적으로 `awf ready --gate workflow-run`를 먼저 통과해야 한다. `approve`와 `done`은 parent-only HIL이므로 `awf wf next --phase approve|done` 및 implicit Done은 `--dry-run`/non-interactive 여부와 관계없이 차단된다. `--dry-run --output-format json`은 지원되는 phase의 prompt preview만 구조화 JSON으로 출력하고 state/prompt 파일을 쓰지 않는다.
...
- `awf wf approve --decision <approve|revise|reject> --actor <audit-label> [--reason <text>] [--repo-root <path>] [--json]`: interactive TTY에서만 실행되며 provider 또는 OMP worker를 실행하지 않고 explicit parent decision만 기록한다. `actor`는 감사 라벨일 뿐 authorization credential이나 worker 권한 위임 수단이 아니다. approve는 현재 sealed plan의 spec/plan/tasks scope hash와 strict `approval.json`, G3/state/history를 기록한다. revise/reject는 `--reason`이 필수이며 secret-like reason은 거부한다. `awf wf next --phase approve`는 승인 대신 차단된다.
- `awf wf confirm --decision <complete|hold> --actor <audit-label> [--pr-url <https://github.com/owner/repo/pull/N>] [--repo-root <path>] [--json]`: interactive TTY에서만 실행되며 G6가 통과하고 현재 phase가 Done일 때 explicit parent Done 결정을 기록한다. `complete`는 strict `confirmation.json`과 Done state/history를 원자적으로 완료 처리하고 동일 완료 기록은 reuse한다. `hold`는 Done을 pending으로 유지하고 state/history에 보류만 기록한다. `--pr-url`은 검증된 GitHub PR URL을 감사 정보로만 저장하며 PR 생성·조회·merge·cleanup·deployment health 판단을 수행하지 않는다. provider/OMP worker/non-interactive path는 이 기록을 만들 수 없다.

`approve`/`confirm`의 TTY와 delegated-phase 차단은 AWF의 parent/worker 경계를
강제하지만 같은 Unix 사용자로 임의 코드를 실행하는 공격자를 cryptographically
격리하지는 않습니다. `actor`는 감사 라벨입니다. Provider/OMP worker는 project
sandbox, isolated write scope와 최소 tool allowlist로 실행해야 하며 신뢰하지 않는
same-UID process에는 workflow directory 접근을 주지 않습니다.
- `awf wf autoresearch-register --result-json <path> [--repo-root <path>] [--json]`: G3가 통과한 Impl phase에서 완료된 OMP Autoresearch result를 등록한다. `--result-json`은 symlink/FIFO/device를 거부하고 nofollow로 여는 64 KiB 이하 regular file이어야 한다. result는 finite numeric score, Planning Options hash, G3 scope hash, allowed changed files, external metrics digest에 결합된다. 동일 canonical result는 reuse하고 다른 current result는 blocked, planning/scope identity 변화는 replan_required다. 이 command는 Autoresearch loop를 실행하지 않으며 score가 G4/G5/G6을 대체하지 않는다.
- `awf wf autoresearch-schema --json`: `autoresearch-register`가 받는 exact versioned JSON Schema를 한 JSON object로 출력한다. OMP loop는 이 schema의 14개 필드(`schema_version`, run/goal/score, baseline/final score, kept candidate, Planning Options/scope identity, metrics path/hash, completion time, changed files)를 채우고 `metrics_path` 본문은 workflow artifact 밖에 둔다. register exit `0`은 written/reuse, `2`는 blocked, `3`은 replan_required다.

Autoresearch envelope와 generic OMP dispatch record는 현재 parent-signed receipt가
아닌 hash/scope-bound self-reported provenance입니다. Done/status의 참고 evidence로만
표시하고 approval, G4/G5/G6 또는 deployment-health 증명으로 사용하지 않습니다.

```bash
awf wf autoresearch-schema --json
awf wf autoresearch-register --result-json .omp/autoresearch/completed.json --repo-root . --json
```
...
- `awf wf seal-plan [--repo-root <path>] [--json]`: writes strict
  `.workflow/artifacts/planning-provenance.json` only for `selected` or
  `no_decision_required` options. JSON exit contracts are sealed/0,
  selection_required-blocked/1, and unavailable-failed/2.
...
- Production primary is never a verify/test benchmark or executable-query target. Production provides only read-only schema metadata; data comes only from an explicitly approved replica, warehouse, or sanitized local dataset. `database-decision.json` defaults to `local_data_test_waiver` as null or omitted. Only an absent `test_command` permits a waiver object with nonempty `reason`, `approver`, and `timestamp`; the timestamp uses UTC ISO 8601. Plan uses `file_write: true` and `workspace-write`. Verify workers and direct Codex Verify use `read-only`; project-owned DB verify commands run in the parent `db-check` boundary. Test keeps `file_write: false` but uses `workspace-write` for Bash-driven commands and test execution.
- `awf wf next`는 stderr에 `result: /actual/result/path`를 출력한다. verify/test gate에는 이 실제 path를 각각 `VERIFY_RESULT` 또는 `TEST_RESULT`에 설정해 `--result-file`로 넘긴다. 고정된 `.workflow/tmp/` filename을 추측하지 않는다.
- DB decision은 `maintain`을 포함한 2~3개의 holistic option을 비교한다. 각 option의 `covered_surfaces`는 decision의 모든 surface와 같고 `surface_assessments`는 각 surface를 nonempty하게 평가한다. `kind`는 dominant strategy일 뿐 surface별 candidate kind를 강제하지 않는다. index surface에서는 각 option이 add, reject, maintain을 비교하며 자동 선택하지 않는다.
- Signal mapping은 table DDL→`erd`, column/index/constraint DDL→동명 surface, schema/type/sequence/trigger/procedure/function→`schema_object`, view DDL→`schema_object`+`query`, migration→structural surface, normalization/denormalization/ERD/keys/partition→matching surface, SQL syntax/order by→`query`다. generic model/models path는 비DB이고 database/models만 DB다. `artifact_error:`는 artifact 수정 전까지 차단하며 signal-derived `requires_query_plan`/`requires_migration_rollback`은 declared surface와 독립적인 hard gate다.
- Verify evidence uses `engine`, `execution_target`, `production_primary_queries`: false, and `raw_production_rows`: false. Structural/index work uses `local_same_engine` with the production schema engine; query planner work may use `approved_read_replica`. DuckDB, cross-engine execution, and the production primary are prohibited.
...
- `awf wt sync --from <production> --to <staging> [--repo-root <path>] [--apply] [--json]`: configured production-only delta를 live merge base부터 latest staging에 exact하게 재적용한다. No-op은 lease/PR을 만들지 않고, source-only patch 적용 뒤 index tree가 target과 같아도 새 managed worktree·local branch를 정리한 verified noop으로 끝낸다. 동일 pin의 clean `sync_apply_failed` lease는 재실행 시 이 cleanup을 복구하지만 drift·dirty·remote branch/PR이 있는 lease는 fail-closed로 보존한다. Clean 3-way merge는 staging-only 변경과 Git mode를 보존하며, 충돌·source/target drift·prepare/production verify 실패도 worktree를 보존한다. clean publication 중단은 같은 verified lease로 재개한다. Reserved sync branch와 `AWF-No-Promote: true`는 promotion/release 재유입을 차단한다.
...
Exact promotion remains the default. Use `--out-of-order` only when one or
more reviewed staging PRs must reach production without unrelated earlier
staging changes. It is never an automatic fallback from exact promotion.
...
The mode accepts one or more repeated `--source-pr` values in staging merge
order and MUST NOT use `--exclude-path`. Every source must be merged into the
configured staging branch and pass its review and checks policy. AWF atomically
pins each source's ordinal, PR number, base ref, base SHA, head SHA, merge SHA,
and reviewed paths in the promotion lease. Those ordered source pins are
authoritative across preview, apply, retry, manual recovery, and publication.

Every source merge SHA must be in staging history, every source base must be an
ancestor of its reviewed head, and each later source merge SHA must descend
from the preceding source merge SHA. Duplicate sources, missing merge SHAs,
reversed merge order, source drift, or a source outside staging stop before
publication. Renamed source paths return `unsupported_out_of_order_rename`.
The ordered list may include a dependent pair such as #527 followed by #530;
a prerequisite outside that list is a stop condition.

Inspect the initial preview before applying. It exposes ordered `sources` pins,
`source_base_sha`, `source_head_sha`, `target_base_sha`, and the reviewed-path
union. AWF applies source patches in input order into one synthetic commit; the
final delta must be a non-empty subset of the ordered reviewed-path union:
...
`stage_paths`, then `commit` after AWF applies any remaining ordered sources,
followed by `verify_production`, `push_branch`, and `open_pull_request`.

`out_of_order_conflict` preserves an unpublished managed worktree with the
complete ordered source pins, target, reviewed-path union, conflicted-path
provenance, and the ordinal of the source that conflicted. Edit only the
conflicted files returned by AWF. Do not use direct `git add`, `git commit`,
`git reset`, `git cherry-pick`, or `git push`.

After staging that resolution, AWF applies each remaining ordered source patch.
If a later source conflicts, its ordinal and protected-index snapshot replace
the prior conflict state. AWF creates the one synthetic commit only after every
source has applied cleanly.
...
of the ordered reviewed-path union.
...
After editing, replay the same ordered preview command and inspect the lease,
conflicted paths, and current changed paths. Use the same command with
`--apply` only when every source pin and target provenance are unchanged. If a
source base, head, merge, reviewed path, order, or target SHA changes, AWF
returns `promotion_provenance_changed` and preserves the worktree rather than
transplanting a resolution. An operator unstaged edit or unmerged path outside
`conflicted_paths` returns `promotion_resolution_scope_mismatch`. AWF stages
the allowed conflict files; an unmerged index entry that remains after staging
returns `promotion_resolution_unmerged`.

All conflict markers must be removed before apply. If a marker remains, AWF
does not publish and preserves the worktree. Commit and PR trailers record each
source PR/base/head/merge in the exact input sequence.
